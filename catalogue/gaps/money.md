...
| M2 | **The Indian rupee sign ₹ is not in Instrument Sans** (B1 gaps A9). | money-transfer-quote, money-transfer-tracking | Amounts use the ISO code ("INR 61,540"); F0's examples used ₹ and are changed here | Renderer: fall back to the ISO code for currencies the face lacks; then examples may use the sign |
...
| M4 | **The share law triggers only on bare percentages.** Subscriptions and unpaid invoices are parts of a total but carry text in `secondary`, so nothing checks their order. | money-subscriptions, money-invoices-owed | Sent largest first by hand; the checker's "rows add up to Total" law does check the sum | none in the contract; a checker rule would need a lead decision on what "a list of parts" means |
