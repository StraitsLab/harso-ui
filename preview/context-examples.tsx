...
export function ContextExample({ state }: { state: ExampleState }) {
  ...
  return <div className="hkl-example-stack">
    <div className="hk-data-toolbar"><label>Context sample<Select value={sample} onChange={event => setSample(event.target.value)}>{["ready", "empty", "loading", "error", "replacement", "disabled"].map(value => <option key={value}>{value}</option>)}</Select></label><Checkbox label="Hold context requests" checked={hold} onChange={event => setHold(event.target.checked)} /></div>
