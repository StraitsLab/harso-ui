...
export function ComposerExample({ component, state }: { component: string; state: ExampleState }) {
  ...
  const attachment = <ComposerAttachmentStrip>{attached && <ComposerAttachmentTile name={name} kind="code" size={2400} status={error ? "error" : status} progress={status === "uploading" ? 0.35 : undefined} message={error ? "Preview attachment rejected" : undefined} onRemove={() => { if (!hold) setAttached(false); }} disabled={disabled} />}</ComposerAttachmentStrip>;
  return <div className="hkl-example-stack">
    ...
    {component === "ComposerLoader" ? <>
      ...
      {component === "ComposerAttachments" ? <ComposerAttachments>{attachment}</ComposerAttachments> : <ComposerWithAttachments loading={loading} error={error ? "The host reports this draft could not be sent." : undefined} value={draft} onValueChange={changeDraft} disabled={disabled} aria-busy={loading || undefined} onSubmit={text => { if (!hold) { setSent(text); setDraft(""); } }}>{attachment}</ComposerWithAttachments>}
    </>}
  </div>;
}
