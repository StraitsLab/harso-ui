import { ArrowLeftIcon, ArrowRightIcon, BellIcon, CompassIcon, PlusIcon } from "@phosphor-icons/react";
...
export function HomeDashboardExample({ state }: { state: ExampleState }) {
  ...
  return <div className="hkl-example-stack" style={{ maxWidth: "none" }}>
    ...
    <HomeDashboard title={state === "long-content" ? "A calm overview of your people, customers and everyday operations" : "Your workspace"} navigation={navigation}
      ...
      actions={<><Button leadingIcon={<CompassIcon size={16} />} disabled={unavailable} onClick={() => act("Navigation opened", () => setPanel("navigation"))}>Navigate</Button><Button leadingIcon={<BellIcon size={16} />} disabled={unavailable} onClick={() => act("Notifications opened", () => setPanel("notifications"))}>Notifications{unread ? " · 1 unread" : ""}</Button><Button leadingIcon={<PlusIcon size={16} />} disabled={unavailable} onClick={() => act("Ticket opened", () => { setTicket(""); setPanel("ticket"); })}>Create ticket</Button></>}
      ...
      hires={overview && <div className="hk-home-hires"><h3>Recent hires</h3><div><strong>{colleagues[hire].name}</strong><p>{colleagues[hire].role} · Joined <time dateTime={colleagues[hire].joined}>{colleagues[hire].joined}</time></p></div><Button leadingIcon={<ArrowLeftIcon size={16} />} disabled={disabled || hire === 0} onClick={() => act("Previous hire", () => setHire(index => index - 1))}>Previous hire</Button><Button leadingIcon={<ArrowRightIcon size={16} />} disabled={disabled || hire === colleagues.length - 1} onClick={() => act("Next hire", () => setHire(index => index + 1))}>Next hire</Button></div>}
