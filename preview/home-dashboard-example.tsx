...
export function HomeDashboardExample({ state }: { state: ExampleState }) {
  ...
  const navigation = <div style={{ position: "sticky", top: 16, margin: 8, width: "fit-content", maxWidth: "calc(100% - 16px)", border: "1px solid var(--hk-line)", borderRadius: 16, background: "var(--hk-surface)", boxShadow: "0 8px 24px rgb(0 0 0 / .08)" }}><Sidebar label="Workspace" collapsed={collapsed} onCollapsedChange={next => act("Sidebar collapse requested", () => setCollapsed(next))}>{["Overview", "Customers"].map(name => <SidebarItem key={name} label={name} disabled={unavailable} selected={section === name} onSelect={() => act("Navigation requested", () => { setSection(name); setPanel(null); })} />)}</Sidebar></div>;
  ...
  return <div className="hkl-example-stack" style={{ maxWidth: "none" }}>
    ...
    <HomeDashboard title={state === "long-content" ? "A calm overview of your people, customers and everyday operations" : "Your workspace"} navigation={navigation}
      ...
      revenue={overview && <LineChartCard disabled={disabled} title="Revenue by customer" value={money.format(revenue)} caption="Synthetic gross amounts, same period" data={source.map(row => ({ label: row.name, value: row.amount * multiplier }))} />}
