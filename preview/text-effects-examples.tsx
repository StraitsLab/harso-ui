...
export function TextEffectsExample({ component: _component, state: _state }: { component: TextEffectsExport; state: ExampleState }) {
  ...
  return <div className="hkc-stack" style={{ gap: 24 }}><Checkbox label="Waiting state" checked={active} onChange={event => setActive(event.target.checked)} /><Field label="Waiting text">{props => <Input {...props} value={waitingText} onChange={event => setWaitingText(event.target.value)} />}</Field><Shimmer as="h2" style={{ margin: 0, fontSize: "var(--hk-text-title)" }} active={active}>{waitingText}</Shimmer><Shimmer as="p" style={{ margin: 0 }} active={active} duration={4} spread={4}>A slower, wider sweep.</Shimmer><Shimmer active={false}>Nothing is running in this example.</Shimmer></div>;
}
