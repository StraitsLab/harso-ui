...
import { Badge, Button, Disclosure, Field, IconButton, Input, KitProvider, Select, filterReferences, referenceAccounting, referenceComponents, referenceSources, retiredReferenceIds, type Appearance, type Palette, type ReferenceComponent, type ReferenceVendor } from "@harso/ui";
...
import { TextEffectsExample, textEffectsExports, textEffectsNotes, type TextEffectsExport } from "./text-effects-examples";
...
function implementedExport(reference: ReferenceComponent): string | undefined {
  return [...foundationExports, ...navigationExports, ...controlExports, ...navigationSurfaceExports, ...dateExports, ...textEffectsExports, ...workExports, ...dataExports, ...activityExports, ...contextExports, ...developerContentExports, ...fileTreeExports, ...commitExports, ...stackTraceExports, ...testResultsExports, ...terminalExports, ...codeBlockExports, ...chainOfThoughtExports, ...attachmentsExports, ...inlineCitationExports, ...confirmationExports, ...catalogueExports].find(name => name === reference.harsoExport);
}
...
function selectionFromHash() {
  return referenceComponents.find(reference => reference.id === routeFromHash())?.id;
}
...
export function Library() {
  ...
  const retired = retiredReferenceIds.includes(route);
  ...
  const textEffectsControl = textEffectsExports.find(name => name === implementation);
  ...
  const supportsLongContent = implementation && ["Notification", "NotificationCenter", "AreaChartCard", "LineChartCard", "ComboChartCard", "TaskList", "WebSearch", "SettingsModal", "ModelSelector", "MicSelector", "VoiceSelector", "Input", "Field", "Breadcrumb", "Announcement", "Tabs", "Dropdown", "Sidebar", "Carousel", "Calendar", ...workExports, ...dataExports, ...activityExports, ...developerContentExports, ...fileTreeExports, ...commitExports, ...stackTraceExports, ...testResultsExports, ...terminalExports, ...codeBlockExports, ...chainOfThoughtExports, ...attachmentsExports].includes(implementation);
  ...
  return <KitProvider appearance={appearance} palette={palette} className="hkl-root">
    ...
    <div className="hkl-layout">
      ...
      <main ref={mainRef} className="hkl-main" id="component-preview" tabIndex={-1}>
        {retired ? <section data-testid="retired-route" role="status"><h1>Retired example</h1><p><code>#{route}</code> is retired — see <a href="#harso:chat-shell">#harso:chat-shell</a> for product chat.</p><p>Profile and image-generation templates have no direct replacement. Their retained primitives remain in the library.</p></section> : <>
