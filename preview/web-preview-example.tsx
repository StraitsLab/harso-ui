...
export function WebPreviewExample({ state }: { state: ExampleState }) {
  ...
  useEffect(() => { const kit = host.current?.closest(".harso-kit"); if (!kit) return; const update = () => setDark(kit.getAttribute("data-mode") === "dark"); update(); const observer = new MutationObserver(update); observer.observe(kit, { attributes: true, attributeFilter: ["data-mode"] }); return () => observer.disconnect(); }, []);
  const [draft, setDraft] = useState("");
  const [url, setUrl] = useState("");
  const [data, setData] = useState(state === "error" ? "rejected" : "document");
  const [locked, setLocked] = useState(false);
  const disabled = state === "disabled" || locked;
  return <div ref={host} className="hkl-example-stack">
    ...
    <WebPreview url={data === "rejected" ? "javascript:void(0)" : data === "address" ? url : ""} disabled={disabled}>
      <WebPreviewNavigation>
        <WebPreviewNavigationButton tooltip="Reload"><ArrowClockwiseIcon size={18} aria-hidden="true" /></WebPreviewNavigationButton>
        <WebPreviewUrl value={draft} placeholder="https://example.com" onChange={event => { event.preventDefault(); setDraft(event.target.value); }} />
        <Button type="submit" disabled={disabled} leadingIcon={<ArrowUpRightIcon size={18} />}>Open</Button>
      </WebPreviewNavigation>
      <WebPreviewBody srcDoc={data === "document" ? (dark ? localDocument.replace("color:#20252a;background:#fafbfd", "color:#f2f3f5;background:#1c1f24") : localDocument) : undefined} loading={<p role="status">Loading preview</p>} />
    </WebPreview>
    </form>
  </div>;
}
