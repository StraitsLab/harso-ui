...
import { Persona, Select, type PersonaState, type PersonaVariant } from "@harso/ui";
...
export function PersonaExample({ state }: { state: ExampleState }) {
  ...
  return <div className="hk-persona-example">
    ...
    <div className="hk-persona-controls">
      ...
      <label><input type="checkbox" checked={paused} onChange={event => setPaused(event.target.checked)} />Pause animation</label>
    </div>
    <Persona variant={variant} state={activity} paused={paused || state === "disabled"} size={160} onReady={() => record("ready")} onPlay={() => record("play")} onPause={() => record("pause")} onStop={() => record("stop")} />
    <output aria-label="Persona lifecycle">{events.join(" · ") || "Waiting for renderer"}</output>
