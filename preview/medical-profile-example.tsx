import { CompassIcon, FileTextIcon } from "@phosphor-icons/react";
...
export function MedicalProfileExample({ state }: { state: ExampleState }) {
  ...
  return <div ref={root} className="hk-medical-example">
    ...
    <MedicalProfile title="A picture of your day" navigation={navigation} actions={<><Button leadingIcon={<CompassIcon size={16} />} disabled={blocked} onClick={() => act("Navigation opened", () => setPanel("navigation"))}>Navigate</Button><Button leadingIcon={<FileTextIcon size={16} />} disabled={blocked || !patient} onClick={() => act("Report editor opened", () => setPanel("report"))}>File a report</Button></>} panel={panel ? { title: panel === "navigation" ? "Health navigation" : panel === "report" ? "File a report" : "Alert detail", content: panel === "navigation" ? navigation : panel === "report" ? report : alertDetail, onClose: closePanel } : null}
      ...
      alerts={<><h3>Important updates</h3>{dataState ?? <ul className="hk-medical-alerts">{alertFixtures.map(item => <li key={item.id}><Notification role="article" title={item.title} description={item.description}><time dateTime={item.date}>{item.date}</time><Button aria-label={`Inspect ${item.title}`} disabled={blocked} onClick={() => act("Alert opened", () => { setAlertId(item.id); setPanel("alert"); })}><span aria-hidden="true">↗</span>Inspect</Button><span>{readAlerts.includes(`${patientId}:${item.id}`) ? "Read" : "Unread"}</span></Notification></li>)}</ul>}</>}
