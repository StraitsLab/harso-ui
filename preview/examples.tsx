...
import { ArrowRightIcon, PlusIcon, ClockIcon, CompassIcon, TrashIcon } from "@phosphor-icons/react";
...
export function FoundationExample({ component, state }: { component: FoundationExport; state: ExampleState }) {
  ...
  switch (component) {
    case "Button": return <div className="hkl-example-stack"><div className="hkl-example-row" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(132px, 1fr))", width: "100%", minWidth: 0, maxWidth: 600, gap: 12 }}><Button variant="primary" disabled={disabled} onClick={onClick}>Continue <ArrowRightIcon size={16} /></Button><Button leadingIcon={<ClockIcon size={16} />} disabled={disabled} onClick={onClick}>Not now</Button><Button variant="outline" leadingIcon={<CompassIcon size={16} />} disabled={disabled} onClick={onClick}>Explore</Button><Button variant="danger" leadingIcon={<TrashIcon size={16} />} disabled={disabled} onClick={onClick}>Remove</Button></div><div className="hkl-example-row" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(132px, 1fr))", width: "100%", minWidth: 0, maxWidth: 600, gap: 12 }}><Button pending>Saving changes</Button><Button disabled>Unavailable</Button></div><output className="hkl-example-output" aria-live="polite">{clicks ? `${clicks} example actions received` : "Try an action. This example does not save anything."}</output></div>;
    ...
    case "Badge": return <div className="hkl-example-row hkl-badge-specimen" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(132px, 1fr))", justifyItems: "start", maxWidth: 720, gap: 12 }}><Badge>Draft</Badge><Badge tone="active">Working</Badge><Badge tone="positive">Complete</Badge><Badge tone="attention">Needs your input</Badge><Badge tone="negative">Couldn’t finish</Badge></div>;
