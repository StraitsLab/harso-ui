...
export function NotificationExample({ component, state }: { component: "Notification" | "NotificationCenter"; state: ExampleState }) {
  ...
  return <div className="hkl-example-stack">
    ...
    {component === "Notification" ? <>
      ...
      <Notification open={visible} title="Work unit updated" description={long ? "The latest result is ready for your inspection. ".repeat(12) : "The latest result is ready."} tone={tone} onDismiss={state === "disabled" ? undefined : () => { if (!hold) setVisible(false); }} />
      {!visible && <p role="status">Notification dismissed.</p>}
      <Button onClick={() => setVisible(true)} disabled={visible}>Show notification</Button>
    </> : <>
      ...
      <Button onClick={() => { setRead([]); setSelected(""); setEmpty(false); }}>Reset notifications</Button>
    </>}
  </div>;
}
