...
export function SocialButtonExample({ state = "default" }: { state?: ExampleState }) {
  ...
  const [appearance, setAppearance] = useState<"colorful" | "black" | "white">("black");
  ...
  return <div className="hkl-example-stack">
    ...
    <div className="hkl-example-row"><SocialButton provider={provider} icon={typeof ProviderMark === "string" ? <img src={ProviderMark} width={20} height={20} alt="" aria-hidden="true" /> : ProviderMark ? <ProviderMark size={20} weight="fill" aria-hidden="true" /> : <span>{provider.slice(0, 1)}</span>} appearance={appearance} size={size} fullWidth={fullWidth} iconOnly={iconOnly} style={!fullWidth && !iconOnly ? { width: 240, maxWidth: "100%" } : undefined} disabled={disabled} pending={pending} onClick={() => request(provider)} /></div>
