...
function PlanExample({ state, showRecords, loading }: { state: ExampleState; showRecords: boolean; loading: boolean }) {
  ...
  return <div className="hkl-example-stack">
    <Plan open={open} disabled={disabled} onOpenChange={next => { setRequests(count => count + 1); if (!refuse) setOpen(next); }} isStreaming={updating}>
      ...
      <PlanFooter><span>{audience ? `Audience: ${audience}` : "A quick choice · Who are we speaking to?"}</span><div className="hk-message-actions"><Button disabled={disabled} aria-pressed={audience === "Our team"} onClick={() => setAudience("Our team")}>Our team</Button><Button disabled={disabled} aria-pressed={audience === "Everyone"} onClick={() => setAudience("Everyone")}>Everyone</Button></div></PlanFooter>
    </Plan>
    <div className="hkl-example-feedback"><Checkbox label="Streaming sample" checked={streaming} onChange={event => setStreaming(event.target.checked)} /><Checkbox label="Keep host disclosure" checked={refuse} onChange={event => setRefuse(event.target.checked)} /><Checkbox label="Show plan details" checked={open} onChange={event => setOpen(event.target.checked)} /><output aria-label="Disclosure requests">{requests} requests</output></div>
  </div>;
}
...
function ReasoningExample({ state, showRecords, loading }: { state: ExampleState; showRecords: boolean; loading: boolean }) {
  ...
  return <div className="hkl-example-stack"><Reasoning open={open} onOpenChange={next => { setRequests(count => count + 1); if (!refuse) setOpen(next); }} disabled={state === "disabled"} isStreaming={(streaming && showRecords) || loading} duration={showRecords && duration.trim() ? Number(duration) : undefined}><ReasoningTrigger getThinkingMessage={state === "long-content" ? () => "Comparing the evidence carefully while preserving every important qualification and uncertainty" : undefined} /><ReasoningContent>{showRecords ? summary : ""}</ReasoningContent><ReasoningState /></Reasoning><div className="hkl-example-feedback"><Checkbox label="Streaming sample" checked={streaming} onChange={event => setStreaming(event.target.checked)} /><Checkbox label="Untrusted summary sample" checked={hostile} onChange={event => setHostile(event.target.checked)} /><Field label="Host elapsed seconds">{props => <Input {...props} value={duration} onChange={event => setDuration(event.target.value)} inputMode="decimal" />}</Field><Button onClick={() => setUpdates(count => count + 1)}>Append sample update</Button><Checkbox label="Keep host disclosure" checked={refuse} onChange={event => setRefuse(event.target.checked)} /><output aria-label="Reasoning disclosure requests">{requests} requests</output></div></div>;
}
...
function LocalCheckpointModes({ disabled, showRecords }: { disabled: boolean; showRecords: boolean }) {
  ...
  return <section aria-label="Local checkpoint modes" className="hkl-example-stack">
    ...
    <div role="group" aria-label="Checkpoint mode" className="hk-message-actions">{(["Manual", "Automatic", "Branching"] as const).map(option => <Button key={option} disabled={disabled} aria-pressed={mode === option} onClick={() => setMode(option)}>{option}</Button>)}</div>
    {mode === "Branching" ? <><p>Two independent supplied branch samples, not a conversation fork.</p><div role="group" aria-label="Local branch" className="hk-message-actions">{(["Main", "Alternative"] as const).map(option => <Button key={option} disabled={disabled} aria-pressed={branch === option} onClick={() => setBranch(option)}>{option}</Button>)}</div></> : <p>{mode === "Manual" ? "Add a marker to the local manual sample explicitly." : "Automatic records are supplied by the sample host, not generated by a clock or runtime."}</p>}
    ...
    <Button disabled={disabled || (mode === "Automatic" && history.markers.length === 2)} onClick={() => setHistories(current => ({ ...current, [historyKey]: { ...current[historyKey], markers: mode === "Automatic" ? ["Automatic marker 1", "Automatic marker 2"] : [...current[historyKey].markers, `${historyKey} marker ${current[historyKey].markers.length + 1}`] } }))}>{mode === "Automatic" ? "Supply automatic sample records" : "Add local marker"}</Button>
