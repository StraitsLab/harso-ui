...
export function AttachmentsExample({ component, state }: { component: AttachmentsExport; state: ExampleState }) {
  const [items, setItems] = useState<AttachmentData[]>(names.map((name, index) => ({ id: String(index + 1), name, url: index ? undefined : sampleImage(0), mediaType: index ? "text/markdown" : "image/png", size: index ? 4096 : 2048 })));
