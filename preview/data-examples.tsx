...
import { Badge, Button, Checkbox, DataTable, Input, Select, StatCards, Table, type DataColumn, type DataSort, type StatItem } from "@harso/ui";
...
function RecordsExample({ component, state }: { component: "Table" | "DataTable"; state: ExampleState }) {
  ...
  const columns: DataColumn<RecordRow>[] = [
    ...
    { id: "actions", label: "Action", align: "end", render: row => <Button size="small" disabled={disabled || row.locked} aria-label={`Review ${row.name}`} onClick={() => setAction(`Review requested for ${row.name}. No file was opened.`)}>Review</Button> },
  ];
