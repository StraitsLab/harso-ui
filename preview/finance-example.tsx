...
export function FinanceExample({ state }: { state: ExampleState }) {
  ...
  return <div className="hkl-example-stack">
    ...
    <FinanceDashboard title={state === "long-content" ? "A clear view of your everyday finances and longer-term plans" : "Your finances"}
      ...
      actions={<Button disabled={disabled || loading || !!error} onClick={() => setRequest("Statement export requested. No file downloaded.")}>Export statement</Button>}
      ...
      cashFlow={overview ? <SankeyChartCard disabled={disabled} title="Cash flow" caption="Synthetic allocation · USD" nodes={[{ name: "Income" }, { name: "Home" }, { name: "Food" }, { name: "Travel" }, { name: "Unspent" }]} links={[...chartData.map(item => ({ source: "Income", target: item.label, value: item.value })), { source: "Income", target: "Unspent", value: 2825 }]} format={value => money.format(value)} /> : undefined}
      ...
      dailySpending={overview ? <HeatmapChartCard disabled={disabled} title="Daily spending" caption="September 1–6 · Synthetic USD amounts" columns={["Sep 1", "Sep 2", "Sep 3", "Sep 4", "Sep 5", "Sep 6"]} rows={[{ label: "Home", values: [1200, 0, 0, 0, 45, 0] }, { label: "Food", values: [0, 68, 0, 6, 0, 32] }, { label: "Travel", values: [0, 0, 24, 0, 0, 0] }]} format={value => `$${value}`} /> : undefined}
