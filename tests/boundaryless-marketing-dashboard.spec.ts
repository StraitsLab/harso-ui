...
for (const width of [1512, 390]) for (const mode of ["light", "dark"] as const) for (const palette of ["clean", "cozy"]) {
  test(`marketing layout ${width} ${mode} ${palette}`, async ({ page }) => {
    ...
    expect(await workspace.evaluate(element => element.querySelector(".hk-marketing-charts")!.getBoundingClientRect().width <= element.querySelector(".hk-ai-workspace-thread")!.clientWidth)).toBe(true);
