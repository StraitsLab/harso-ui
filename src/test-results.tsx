...
export function TestResultsProgress({ className = "", ...props }: DivProps) { const { summary, isStreaming } = useResults(); const valid = validSummary(summary) && summary!.total > 0; return <div {...props} className={`hk-test-results-progress ${className}`}>{valid ? <Progress label={isStreaming ? "Tests completed" : "Test completion"} value={summary!.passed + summary!.failed + summary!.skipped} max={summary!.total} /> : isStreaming ? <span role="status">Waiting for test results</span> : null}</div>; }
