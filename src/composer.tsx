...
export function ComposerPanel({ children, className = "", value = "", onValueChange, onSubmit, disabled = false, loading = false, error, placeholder = "Ask anything", permission, modelPicker, attachments, context, onVoiceRequest, status, onFilesSelected, accept, ...props }: ComposerPanelProps) {
  ...
  return <div {...props} className={`hk-composer-panel ${className}`} aria-busy={loading || undefined} aria-disabled={disabled || undefined}>
    ...
    {children ?? <Composer value={value} onValueChange={onValueChange} onSubmit={onSubmit} disabled={locked}>
      ...
      <GlassComposer>
        ...
        <PromptInputFooter>
          {onFilesSelected && <label className="hk-composer-file-action">Attach<input type="file" aria-label="Attach files" multiple accept={accept} disabled={locked} onChange={event => { const files = Array.from(event.currentTarget.files ?? []); event.currentTarget.value = ""; if (!locked && files.length) onFilesSelected(files); }} /></label>}
          ...
          {onVoiceRequest && <Button size="small" disabled={locked} aria-label="Request voice input" onClick={() => { if (!locked) onVoiceRequest(); }}>Voice</Button>}
          <PromptInputSubmit disabled={locked || !onSubmit} />
        </PromptInputFooter>
      </GlassComposer>
    </Composer>}
    <ComposerLoader active={loading} />
...
export function ComposerWithAttachments({ children, ...props }: ComponentPropsWithRef<typeof Composer>) { return <Composer {...props}><ComposerAttachments>{children}</ComposerAttachments><GlassComposer><PromptInputTextarea aria-label="Message" placeholder="Ask anything" disabled={props.disabled} /><PromptInputFooter><PromptInputSubmit disabled={props.disabled || !props.onSubmit} /></PromptInputFooter></GlassComposer></Composer>; }
...
export function ComposerAttachmentTile({ name, kind, status, progress, message, onRemove, disabled = false, className = "", ...props }: ComponentPropsWithRef<"div"> & { name: string; kind?: "image" | "document" | "spreadsheet" | "presentation" | "code" | "video"; status?: FileUploadItem["status"]; progress?: number; message?: string; onRemove?: () => void; disabled?: boolean }) {
  const amount = progress !== undefined && Number.isFinite(progress) ? Math.max(0, Math.min(1, progress)) : undefined;
  return <div {...props} className={`hk-composer-attachment-tile ${className}`} data-kind={kind}><span aria-hidden="true">↳</span><span>{name}</span>{kind && <span className="hk-composer-attachment-kind">{kind}</span>}{status && <span>{({ selected: "Queued", uploading: "Uploading", complete: "Landed", error: "Failed" })[status]}</span>}{status === "uploading" && <><svg className="hk-composer-progress-ring" viewBox="0 0 24 24" aria-hidden="true" focusable="false" data-indeterminate={amount === undefined || undefined}><circle className="hk-composer-progress-track" cx="12" cy="12" r="9" />{amount !== undefined && <circle className="hk-composer-progress-amount" cx="12" cy="12" r="9" pathLength="100" strokeDasharray="100" strokeDashoffset={100 - amount * 100} />}</svg><Progress label={`Upload progress for ${name}`} value={amount} />{amount !== undefined && <span>{Math.round(amount * 100)}%</span>}</>}{message && <span role={status === "error" ? "alert" : undefined}>{message}</span>}{onRemove && <Button size="small" disabled={disabled} aria-label={`Remove ${name}`} onClick={() => { if (!disabled) onRemove(); }}>Remove</Button>}</div>;
}
...
export function ModelPicker({ children, className = "", models, providers = [], efforts = [], value, provider, effort, query = "", onValueChange, onProviderChange, onEffortChange, onQueryChange, loading = false, error, disabled = false, onClick, ...props }: ModelPickerProps) {
  ...
  const locked = disabled || loading;
  if (models === undefined) return <button {...props} onClick={onClick} disabled={locked} type="button" className={`hk-model-picker ${className}`}>{children ?? "Choose model"}</button>;
  ...
  return <div ref={root} className="hk-composer-model-control" onKeyDown={event => { if (event.key === "Escape" && !event.defaultPrevented) { event.stopPropagation(); setOpen(false); root.current?.querySelector("button")?.focus(); } }}>
    <button {...props} type="button" disabled={locked} aria-expanded={open && !locked} aria-controls={identity} className={`hk-model-picker ${className}`} onClick={event => { onClick?.(event); if (!event.defaultPrevented && !locked) setOpen(previous => !previous); }}>{children ?? models.find(model => model.id === value)?.label ?? "Choose model"}</button>
