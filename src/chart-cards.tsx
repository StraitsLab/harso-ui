...
function BarChart({ data, title, active, inspect, showIcons, comparison = false }: ChartPlotProps) {
  ...
  return <>{comparison && <div className="hk-chart-comparison-legend" aria-label={`${title} series`}><span>Current</span><span>Previous</span></div>}<div className="hk-chart-bars" role="group" tabIndex={0} aria-label={`${title} chart`}>{data.map((item, index) => {
...
function FunnelChart({ data, title, active, inspect, showIcons, limit, display = "value", tapered = false, shape = "eased" }: ChartPlotProps) {
  ...
  return <div className="hk-chart-funnel" role="group" aria-label={`${title} chart`}>{data.slice(0, limit ?? data.length).map((item, index) => {
...
function ChartSvg({ data, kind, title, active, inspect, onInspect, radarVariant = "filled", radialLayout = "rings", score, showTiles = false }: ChartPlotProps & ChartAnatomyProps & { kind: string }) {
  ...
  const metricList = <ul className={`hk-chart-metrics${showTiles ? " hk-chart-stat-tiles" : ""}`}>{data.map((item, index) => <li key={index}><button {...inspect(index)} data-active={active === index}><span>{kind === "radar" && <span className="hk-radar-category-number" aria-hidden="true">{index + 1} · </span>}{kind === "radar" && !item.label.trim() ? "Unlabeled category" : item.label}</span><strong>{valid[index] ? kind === "radial" ? `${item.value}%` : item.value : "Unavailable"}</strong></button></li>)}</ul>;
  if (kind === "radar") return <>
    <div className="hk-radar-plot" data-variant={radarVariant}>
      <svg className="hk-chart-svg" viewBox="0 0 108 108" role="img" aria-label={`${title} chart`}>
        ...
        {points.map((point, index) => <text key={index} className="hk-radar-axis-label" data-active={active === index} x={54 + (point.axisX - 54) * 49 / 43} y={54 + (point.axisY - 54) * 49 / 43} aria-hidden="true">{index + 1}</text>)}
      </svg>
  ...
  const radial = (item: ChartDatum, index: number) => {
    ...
    const color = item.color ?? ["var(--hk-accent)", "var(--hk-positive)", "var(--hk-secondary)"][index % 3];
...
export function ActivityRingsCard({ title, caption, value, rings = [], disabled = false, calendar, targetLabel = "Goal", children, className = "", ...props }: ActivityRingsProps) {
  ...
  const colors = ["var(--hk-accent)", "var(--hk-positive)", "var(--hk-secondary)"];
  ...
  return <article {...props} className={`hk-chart-card hk-heatmap ${className}`} style={{ "--hk-heatmap-color": color, ...props.style } as CSSProperties} onKeyDown={event => { props.onKeyDown?.(event); if (!event.defaultPrevented && event.key === "Escape") setInspection(null); }}>
    ...
    {children ?? (error ? <p role="alert">{error}</p> : <>
      ...
      {rows.length && columns.length ? <div className="hk-heatmap-scroll" role="region" aria-label={`${title} matrix`} tabIndex={0} onPointerLeave={event => { if (event.pointerType !== "touch") setInspection(focused.current); }}>
        <table aria-label={`${title} values`}><thead><tr><th scope="col">Series</th>{columns.map(column => <th key={column} scope="col" data-active={active?.column === column}>{column}</th>)}</tr></thead>
...
export function SankeyChartCard({ title, caption, value, nodes = [], links = [], format = String, range = "", ranges = [], onRangeChange, disabled = false, children, className = "", ...props }: SankeyChartProps) {
  ...
  return <article {...props} className={`hk-chart-card hk-sankey ${className}`} onKeyDown={event => { props.onKeyDown?.(event); if (!event.defaultPrevented && event.key === "Escape") setInspection(null); }}>
    ...
    {children ?? (typeof graph === "string" ? <p role="alert">{graph}</p> : <>
      ...
      {graph.links.some(link => link.original.value > 0) ? <div className="hk-sankey-scroll" tabIndex={0} role="region" aria-label={`${title} diagram`}><svg viewBox={`0 -16 ${graph.width} ${graph.height + 32}`} style={{ minWidth: graph.width }} role="img" aria-label={`${title} chart`} onPointerLeave={event => { if (event.pointerType !== "touch") setInspection(null); }}>
        <title>{graph.links.map(linkLabel).join("; ")}</title>
        {graph.links.map((link, index) => link.original.value > 0 && <path key={index} className="hk-sankey-link" d={link.path} fill="none" stroke={link.source.color ?? colors[link.source.index % colors.length]} strokeWidth={link.width} opacity={!active || active === link.original || active === link.source.name || active === link.target.name ? .55 : .1} onPointerEnter={() => focus(link.original)} onPointerDown={() => focus(link.original)}><title>{linkLabel(link)}</title></path>)}
        {graph.nodes.map(node => <g key={node.name} onPointerEnter={() => focus(node.name)} onPointerDown={() => focus(node.name)}><rect x={node.x} y={node.y} width="12" height={node.height} fill={node.color ?? colors[node.index % colors.length]} /><text x={node.x > graph.width / 2 ? node.x - 6 : node.x + 18} y={node.y + node.height / 2} textAnchor={node.x > graph.width / 2 ? "end" : "start"} dominantBaseline="middle">{node.name.length > 22 ? `${node.name.slice(0, 21)}…` : node.name}<title>{nodeLabel(node)}</title></text></g>)}
      </svg></div> : <p>No positive flows supplied.</p>}
      <div className="hk-sankey-details"><section aria-label="Flow nodes"><h4>Nodes</h4>{graph.nodes.map(node => <Button key={node.name} disabled={disabled} onFocus={() => focus(node.name)} onBlur={() => setInspection(null)} onClick={() => focus(node.name)}>{nodeLabel(node)}{node.outgoing === 0 && node.incoming > 0 && graph.sinkTotal > 0 ? ` · ${(node.incoming / graph.sinkTotal * 100).toFixed(1).replace(/\.0$/, "")}% of sinks` : ""}</Button>)}</section><section aria-label="Flow links"><h4>Links</h4>{graph.links.map((link, index) => <Button key={index} disabled={disabled} onFocus={() => focus(link.original)} onBlur={() => setInspection(null)} onClick={() => focus(link.original)}>{linkLabel(link)}</Button>)}</section></div>
    </>)}
  </article>;
}
...
export function ScatterChartCard({ title, caption, value, series = [], bubble = true, tiles = true, axisLabels = { x: "X", y: "Y" }, format = String, formatX = String, ranges = [], range = "", onRangeChange, disabled = false, children, className = "", ...props }: ScatterChartProps) {
  ...
  return <article {...props} className={`hk-chart-card hk-scatter ${className}`}>
    ...
    {children ?? <>
      ...
      {points.length ? <svg viewBox="0 0 360 236" className="hk-scatter-plot" role="group" aria-label={`${title} chart`} onKeyDown={event => { if (event.key === "Escape") { setInspection(null); event.stopPropagation(); } }}>
        <path d="M48 18 V196 H336" className="hk-chart-grid" />
        <text x="192" y="231" textAnchor="middle">{axisLabels.x}</text><text transform="translate(12 107) rotate(-90)" textAnchor="middle">{axisLabels.y}</text>
        <text x="48" y="214">{formatX(minX)}</text><text x="336" y="214" textAnchor="end">{formatX(maxX)}</text>
        <text x="44" y="196" textAnchor="end">{format(minY)}</text><text x="44" y="24" textAnchor="end">{format(maxY)}</text>
        {groups.map((group, groupIndex) => <g key={`${group.label}-${groupIndex}`} opacity={active && active.series !== group.label ? .3 : 1}>{group.points.map((point, pointIndex) => <circle key={`${point.label ?? "point"}-${pointIndex}`} cx={48 + position(point.x, minX, maxX) * 288} cy={196 - position(point.y, minY, maxY) * 178} r={bubble ? Math.sqrt(9 + (maximumSize ? (point.z ?? 1) / maximumSize : 0) * 135) : 5} fill={group.color ?? colors[groupIndex % colors.length]} role="img" aria-label={pointLabel(group.label, point)} tabIndex={disabled ? -1 : 0} onFocus={() => { if (!disabled) setInspection({ series: group.label, point }); }} onBlur={() => setInspection(null)} onPointerEnter={() => { if (!disabled) setInspection({ series: group.label, point }); }} onPointerLeave={event => { if (event.pointerType !== "touch" && document.activeElement !== event.currentTarget) setInspection(null); }} onPointerDown={() => { if (!disabled) setInspection({ series: group.label, point }); }}><title>{pointLabel(group.label, point)}</title></circle>)}</g>)}
      </svg> : <p className="hk-chart-empty">No valid observations.</p>}
...
export function MostActiveDaysCard({ days = [], data, selectedDate, onSelectedDateChange, month, onMonthChange, months, today, disabled = false, loading = false, error, children, ...props }: MostActiveDaysCardProps) {
  ...
  return <ChartCard {...props} disabled={disabled} loading={loading} error={error} className={`hk-chart-card--days ${props.className ?? ""}`}>{children ?? (error ? <p role="alert">{error}</p> : invalid ? <p role="alert">Activity records require valid unique dates and months.</p> : loading ? <p role="status">Loading activity…</p> : <>
    <div className="hk-active-days-scroll" role="region" aria-label={`${props.title} months`} tabIndex={0}>{months ? months.map(period => <MonthPanel key={period} month={period} minDate={`${period}-01`} maxDate={lastDate(period)} value={selected} onValueChange={select} today={today} disabled={disabled || (selectedDate !== undefined && !onSelectedDateChange)} />) : <MonthPanel month={month} defaultMonth={days[0]?.date.slice(0,7)} onMonthChange={onMonthChange} value={selected} onValueChange={select} today={today} disabled={disabled || (selectedDate !== undefined && !onSelectedDateChange)} />}</div>
