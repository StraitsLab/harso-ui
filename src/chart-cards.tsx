...
export function HeatmapChartCard({ title, caption, value, rows = [], columns = [], max, color = "var(--hk-accent)", format = String, delta, disabled = false, range = "", ranges = [], onRangeChange, children, className = "", ...props }: HeatmapChartProps) {
  ...
  const matrixCard = useRef<HTMLElement>(null);
  useEffect(() => { const card = matrixCard.current; if (!card || typeof ResizeObserver === "undefined") return; const update = () => setNarrow(card.clientWidth < (columns.length + 1) * 88); update(); const observer = new ResizeObserver(update); observer.observe(card); return () => observer.disconnect(); }, [columns.length]);
  ...
  return <article {...props} ref={matrixCard} className={`hk-chart-card hk-heatmap ${className}`} style={{ "--hk-heatmap-color": color, ...props.style } as CSSProperties} onKeyDown={event => { props.onKeyDown?.(event); if (!event.defaultPrevented && event.key === "Escape") setInspection(null); }}>
