...
export function PromptInputSubmit({ className = "", children = "Send", ...props }: ButtonProps) { const prompt = usePrompt(); return <Button {...props} type="button" size="small" disabled={props.disabled || !prompt.canSubmit} className={`hk-prompt-submit ${className}`} onClick={event => { props.onClick?.(event); if (!event.defaultPrevented) prompt.submit(); }}>{typeof children === "string" ? <><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7" /></svg><span className="hk-sr-only">{children}</span></> : children}</Button>; }
