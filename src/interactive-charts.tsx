...
export function InteractiveChart({ kind, title, value, caption, data = empty, series, bar, line, variant = "stacked", shape = "curved", ranges = [], range, onRangeChange, headline, delta, format = String, tiles = false, disabled = false, loading = false, error, children, className = "", ...props }: InteractiveChartProps & { kind: "area" | "line" | "combo" }) {
  ...
  return <article {...props} aria-busy={loading || undefined} className={`hk-chart-card hk-chart-card--${kind} hk-interactive-chart ${className}`}>
    ...
    {children ?? (failure ? <p role="alert">{failure}</p> : loading ? <p role="status">Loading chart…</p> : !rows.length ? <div className="hk-chart-empty">No data</div> : <>
      ...
      <div className="hk-interactive-ticks" aria-hidden="true">{plots.filter((_, index) => kind === "combo" || index === 0).map((plot, index) => <div key={index} className={index ? "hk-axis-right" : "hk-axis-left"}>{[1, .5, 0].map(level => <span key={level} style={{top: `${(20 + (1 - level) * 164) / 192 * 100}%`}}>{percent ? `${level * 100}%` : text(Number(((plot.minimum + (plot.maximum - plot.minimum) * level) * plot.scale).toPrecision(4)), groups[index])}</span>)}</div>)}</div>
      <svg className="hk-interactive-plot" preserveAspectRatio="none" viewBox="0 0 440 192" role="img" aria-label={`${title} chart`} onPointerLeave={event => { if (event.pointerType !== "touch" && !buttons.current.includes(document.activeElement as HTMLButtonElement)) setInspection(null); }}>
