var APP="Light/Clean";var CFG=["AreaChartCard", "area", "Traffic composition", "Synthetic fixtures \u00b7 each period supplied by this host; no fetched or generated period data", ["Organic", "Referral", "Paid"]];
...
// Non-text series use separated existing neutral tokens; text roles are unchanged.
var greyDarkRole=APP.startsWith('Dark/')?'faint':'secondary',greyLightRole=APP.startsWith('Dark/')?'secondary':'faint';
C.greyDark=H.hex(APP,greyDarkRole);C.greyLight=H.hex(APP,greyLightRole);
function plot(kind){
var s=[],W=960,HH=210,a=C.accent,b=C.secondary,c=C.tertiary,g=C.line;
if(['area','sleep'].includes(kind)){b=C.greyDark;c=C.greyLight;}
...
[.333,.667,1].forEach(v=>poly([v,v,v,v],g));points([1,1,1,1]).forEach(p=>path('M480 105L'+p.join(' '),g,1));poly([0,.0025,.75,1],a,a+'22');points([0,.0025,.75,1]).forEach((p,i)=>s.push('<circle id="vertex-'+i+'" cx="'+p[0]+'" cy="'+p[1]+'" r="2" fill="'+a+'"/>'));['Speed','Quality','Care','Reach'].forEach((v,i)=>{var p=points([1.24,1.6,1.24,1.5])[i];text(p[0],p[1]+4,v,'middle');});text(480,229,'Guides    33.3    66.7    100','middle');}
if(kind==='sleep'){
...
if(kind==='area')legend([['Organic · total: 12,500','accent'],['Referral · total: 4,000',greyDarkRole],['Paid · total: 1,850',greyLightRole]]);
...
if(kind==='sleep')legend([['Stage 1: 0 / 1','accent'],['Stage 2: 0.25 / 1',greyDarkRole],['Stage 3: 0.5 / 1',greyLightRole]],true);
...
function detail(parent,v,w){var r=frame(parent,'inspect detail','row',w,8);r.stackLayout.alignItems=sketch.StackLayout.AlignItems.Center;txt(r,v,12,'secondary',w-24);H.icon({parent:r,name:'inspect arrow',d:H.paths.arrowRight,size:16,color:C.tertiary});H.order(r);}
if(kind==='sankey'){var lists=frame(card,'node and link breakdown','row',960,24);var node=frame(lists,'nodes','col',510,6),links=frame(lists,'links','col',426,6);txt(node,'Nodes',12,'ink',510,6);txt(links,'Links',12,'ink',426,6);['Salary · In: $0 · Out: $2,400','Freelance · In: $0 · Out: $600','Budget · In: $3,000 · Out: $3,000','Home · In: $1,200 · Out: $0 · 40% of sinks','Everyday · In: $800 · Out: $0 · 26.7% of sinks','Savings · In: $1,000 · Out: $0 · 33.3% of sinks'].forEach(v=>detail(node,v,510));['Salary → Budget: $2,400','Freelance → Budget: $600','Budget → Home: $1,200','Budget → Everyday: $800','Budget → Savings: $1,000'].forEach(v=>detail(links,v,426));H.order(node);H.order(links);H.order(lists);}
