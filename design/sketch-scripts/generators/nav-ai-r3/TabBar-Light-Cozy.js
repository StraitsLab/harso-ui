...
var prior=page.layers.find(l=>l.name===FAMILY+' — '+APP);if(!prior)throw Error('Missing sheet');var oldX=prior.frame.x,oldY=prior.frame.y;H.dropSheet('Navigation',FAMILY+' — '+APP);var existing=null;var sh=H.sheet({name:FAMILY+' — '+APP,parent:page,x:oldX,y:oldY,fill:H.sw(APP,'canvas-deep')});
...
var s=specimen('Floating · five tabs');var c=fr(s.r,'TabBar/'+APP+'/default','row',374,64,6,2,'surface',999);c.style.borders=[{color:H.sw(APP,'line'),thickness:1}];['Home','Search','Projects','Inbox','Profile'].forEach((t,i)=>{var tab=fr(c,'tab','col',70,52,6,3,i===0?'accent-soft':null,999);tab.stackLayout.alignItems=sketch.StackLayout.AlignItems.Center;var slot=fr(tab,'icon slot','row',58,16,0,0);slot.stackLayout.justifyContent=sketch.StackLayout.JustifyContent.Center;ic(slot,['home','search','folder','mail','user'][i],16,i===0?'accent':'secondary');done(slot);var label=tx(tab,t,12,i===0?'accent':'secondary',58,6);label.style.alignment=sketch.Text.Alignment.center;done(tab);});done(c);end(s);
