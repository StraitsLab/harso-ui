...
function menu(p,name,selection,submenuOpen){var m=card(p,name,244,6,12); m.style.shadows=[{color:'#0000001f',blur:24,x:0,y:8,spread:0}];tx(m,selection?'View options':'Conversation',12,'tertiary');if(selection){row(m,'Show sidebar','check',232,true);row(m,'Show details','check',232);line(m,232,36);row(m,'Comfortable','circleCheck',232,true);row(m,'Compact','circle',232);}else{row(m,'Rename','pencil',232,false,'⌘R');row(m,'Duplicate','copy',232,!submenuOpen,'⌘D');row(m,'Move to project','folder',232,!!submenuOpen,'›');line(m,232,36);row(m,'Export','download',232,false,null,0,'tertiary');row(m,'Delete','trash',232,false,'⌫',0,'negative');}return done(m);} if(!existing){var s=specimen('Default');menu(s.r,'Menu/'+APP+'/default',false);end(s);}var s=specimen('Checkmarks and persistent radio selection');menu(s.r,'Menu/'+APP+'/selection',true);end(s);var s=specimen('Submenu open');var wrap=fr(s.r,'Menu/'+APP+'/submenu/open','row',undefined,undefined,0,4);wrap.stackLayout.alignItems=sketch.StackLayout.AlignItems.Start;var parentMenu=menu(wrap,'main menu',false,true);var fly=fr(wrap,'flyout anchor','col',220,undefined,0,0);var trigger=parentMenu.layers.find(l=>l.name==='row'&&(l.layers||[]).some(t=>t.type==='Text'&&t.text==='Move to project'));spacer(fly,trigger?trigger.frame.y:111);var sub=card(fly,'submenu',220,6,12);tx(sub,'Move to project',12,'tertiary');row(sub,'Research studio','folder',208,true);row(sub,'Product design','folder',208);row(sub,'New project','plus',208);done(sub);done(fly);done(wrap);end(s);
