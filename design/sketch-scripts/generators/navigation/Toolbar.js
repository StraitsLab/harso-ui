...
['default','overflow-open'].forEach(v=>{var s=specimen(v);var c=fr(s.r,'Toolbar/'+APP+'/'+v,'col',760,undefined,0,8);var t=fr(c,'toolbar','row',760,48,{left:8,right:8,top:0,bottom:0},8,'surface',12);iconbtn(t,'arrowLeft',28);iconbtn(t,'chevronRight',28);var title=fr(t,'title','col',undefined,undefined,0,1);tx(title,'Research studio',14,'ink',undefined,6);tx(title,'Edited',12,'tertiary');done(title);flex(t);['panelLeft','upload','dots'].forEach(k=>iconbtn(t,k));done(t);if(v==='overflow-open'){var align=fr(c,'menu alignment','row',760,undefined,0,0);flex(align);var m=card(align,'overflow menu',220,6,16);row(m,'Publish','upload',208);row(m,'Duplicate','copy',208);line(m,208,34);row(m,'Remove','trash',208,false,null,0,'negative');done(m);done(align);}done(c);end(s);});
