...
var s=specimen('Search · grouped results');var c=sym(s.r,'default',560,8,16);var h=fr(c,'search','row',544,48,12,12);ic(h,'search');tx(h,'Search commands or projects…',15,'tertiary');flex(h);key(h,'⌘K');done(h);line(c,544,12);tx(c,'Projects',12,'tertiary',undefined,6);row(c,'Research studio','folder',544,true,'↵');row(c,'Product design','folder',544,false,'⌘1');row(c,'Working note','file',544,false,'⌘2');line(c,544,36);tx(c,'Actions',12,'tertiary',undefined,6);row(c,'Create project','plus',544,false,'⌘N');row(c,'Open settings','settings',544,false,'⌘,');line(c,544,12);var f=fr(c,'hints','row',544,28,12,8);key(f,'↑↓');tx(f,'Navigate',12,'tertiary');key(f,'↵');tx(f,'Open',12,'tertiary');flex(f);key(f,'esc');tx(f,'Close',12,'tertiary');done(f);done(c);end(s);
