...
['medium','full'].forEach(v=>{var s=specimen(v+' detent');var c=sym(s.r,v,390,20,20);c.verticalSizing=sketch.FlexSizing.Fixed;c.frame.height=v==='full'?760:440;var g=fr(c,'grabber area','row',350,8,0);g.stackLayout.justifyContent=sketch.StackLayout.JustifyContent.Center;H.rect({parent:g,w:36,h:5,radius:999,fill:H.sw(APP,'tertiary')});done(g);var h=fr(c,'header','row',350,44,0,8);tx(h,'Project details',20,'ink',undefined,6);flex(h);iconbtn(h,'x');done(h);tx(c,'Research studio',16,'ink',350,6);tx(c,'Keep the important details close.',14,'secondary',350);line(c,350,0);row(c,'Product design','folder',350);row(c,'Working note','file',350);if(v==='full'){tx(c,'Activity',12,'tertiary');row(c,'Changes saved','circleCheck',350);row(c,'Edited just now','clock',350);}var sp=fr(c,'flexible content','col',1,1);sp.verticalSizing=sketch.FlexSizing.Fill;btn(c,'Done',true,350);done(c);end(s);});
