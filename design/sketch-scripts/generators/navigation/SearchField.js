...
['default','typing','clear'].forEach(v=>{var s=specimen(v);var c=fr(s.r,'SearchField/'+APP+'/'+v,'row',360,36,{left:14,right:14,top:0,bottom:0},8,'surface',999);c.style.borders=[{color:H.sw(APP,v==='typing'?'accent':'control-line'),thickness:1}];ic(c,'search');tx(c,v==='default'?'Search…':v==='typing'?'Research|':'Research studio',14,v==='default'?'tertiary':'ink');flex(c);if(v==='clear')ic(c,'x');else key(c,'⌘K');done(c);end(s);});
