...
['top','bottom','left','right'].forEach(function(pos){var s=specimen('Arrow · '+pos);var root=fr(s.r,'Popover/'+APP+'/'+pos,'col',340,210);root.stackLayout=null;var body=card(root,'surface',320,16,12);tx(body,'Share this project',16,'ink',288,6);tx(body,'Invite your team to collaborate in Research studio.',14,'secondary',288);var actions=fr(body,'actions','row',288,36,0,8);btn(actions,'Cancel',false);btn(actions,'Copy link',true);done(actions);done(body);H.relayout(body);body.frame.x=10;body.frame.y=10;var a=H.rect({parent:root,name:'arrow',w:8,h:8,fill:H.sw(APP,'surface')});a.transform.rotation=45; a.frame.x=pos==='left'?6:pos==='right'?326:166;a.frame.y=pos==='top'?6:pos==='bottom'?body.frame.height+6:70;end(s);});
