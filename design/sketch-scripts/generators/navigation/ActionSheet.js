...
var s=specimen('Grouped actions');var c=fr(s.r,'ActionSheet/'+APP+'/default','col',390,undefined,8,8);var actions=card(c,'actions',374,8,16);tx(actions,'Project actions',12,'tertiary',358);['Share project','Duplicate','Move to project','Delete project'].forEach((t,i)=>{var r=fr(actions,'action','row',358,44,8,8);ic(r,['upload','copy','folder','trash'][i],16,i===3?'negative':'secondary');tx(r,t,14,i===3?'negative':'ink');done(r);if(i<3)line(actions,358,32);});done(actions);var cancel=card(c,'cancel',374,8,16);btn(cancel,'Cancel',false,358);done(cancel);done(c);end(s);
