...
function axes(labels,max){[0,1,2,3].forEach(i=>{var y=180-i*48;path('M36 '+y+'H386',g,1);text(27,y+4,((max||6000)>=1000?String((max||6000)*i/3000)+'k':String(Math.round((max||6000)*i/3*100)/100)),'end');});labels.forEach((l,i)=>text(48+i*326/(labels.length-1),204,l,'middle'));}
...
path(d,a,3);
if(kind==='area'){path('M48 155 C100 150 130 136 153 142 S220 160 263 145 S331 119 374 112',b,2);path('M48 170 C100 168 130 160 153 165 S220 174 263 160 S331 149 374 147',c,2);}
...
if(kind==='steps'){path('M36 60H386',c,1.5,'none','stroke-dasharray="5 4"');rect(272,6,116,22,C.surface);text(383,22,'Goal 10,000','end',C.secondary);}
}
if(kind==='barlist'){
...
var sh=sketch.find('#107C908D-BB3E-43CD-BF8B-AD194E9AF068',doc)[0]; var card=sh.layers.find(l=>l.type==='SymbolMaster');var old=card.layers.find(l=>l.name==='vector plot');var idx=old.index;var l=sketch.createLayerFromData(plot("steps"),'svg');l.name='vector plot';l.parent=card;l.frame.width=400;l.frame.height=212;l.index=idx;old.remove();H.relayout(sh);H.out({sheet:String(sh.id),symbol:String(card.id),plot:String(l.id),children:card.layers.length});