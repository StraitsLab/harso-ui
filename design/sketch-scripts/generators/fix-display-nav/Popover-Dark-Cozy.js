...
['top','bottom','left','right'].forEach(function(pos){var s=specimen('Arrow · '+pos);var root=fr(s.r,'Popover/'+APP+'/'+pos,'col',340,210);root.stackLayout=null;var body=card(root,'surface',320,16,12);tx(body,'Share this project',16,'ink',288,6);tx(body,'Invite your team to collaborate in Research studio.',14,'secondary',288);var actions=fr(body,'actions','row',288,36,0,8);btn(actions,'Cancel',false);btn(actions,'Copy link',true);done(actions);done(body);H.relayout(body);body.frame.x=10;body.frame.y=10;var paths={top:'M0 8 L8 0 L16 8',bottom:'M0 0 L8 8 L16 0',left:'M8 0 L0 8 L8 16',right:'M0 0 L8 8 L0 16'};var vertical=pos==='top'||pos==='bottom';var svg='<svg xmlns="http://www.w3.org/2000/svg" width="'+(vertical?16:8)+'" height="'+(vertical?8:16)+'"><path d="'+paths[pos]+' Z" fill="'+H.hex(APP,'surface')+'"/><path d="'+paths[pos]+'" fill="none" stroke="'+H.hex(APP,'line')+'" stroke-width="1"/></svg>';var a=sketch.createLayerFromData(svg,'svg');a.name='merged pointer';a.parent=root;a.frame.x=pos==='left'?2:pos==='right'?330:162;a.frame.y=pos==='top'?2:pos==='bottom'?body.frame.height+10:70;H.rect({parent:root,name:'base border knockout',x:pos==='left'?9:pos==='right'?329:163,y:pos==='top'?9:pos==='bottom'?body.frame.height+9:71,w:vertical?14:2,h:vertical?2:14,fill:H.sw(APP,'surface')});end(s);});
