...
var p=H.page('Proto iOS Dark');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/signin'||(/^caption\//.test(l.name)&&l.name.endsWith('/signin')))l.remove();});build('signin',0);