...
var p=H.page('Proto iOS Dark');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/conversation--approval'||(/^caption\//.test(l.name)&&l.name.endsWith('/conversation--approval')))l.remove();});build('conversation--approval',3);