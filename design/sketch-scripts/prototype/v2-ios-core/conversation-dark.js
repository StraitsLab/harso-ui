...
var p=H.page('Proto iOS Dark');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/conversation'||(/^caption\//.test(l.name)&&l.name.endsWith('/conversation')))l.remove();});build('conversation',2);