...
var p=H.page('Proto iOS Light');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/activity'||(/^caption\//.test(l.name)&&l.name.endsWith('/activity')))l.remove();});build('activity',7);