...
var p=H.page('Proto iOS Light');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/conversation--delete'||(/^caption\//.test(l.name)&&l.name.endsWith('/conversation--delete')))l.remove();});build('conversation--delete',5);