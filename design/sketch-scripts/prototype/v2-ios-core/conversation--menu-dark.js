...
var p=H.page('Proto iOS Dark');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/conversation--menu'||(/^caption\//.test(l.name)&&l.name.endsWith('/conversation--menu')))l.remove();});build('conversation--menu',4);