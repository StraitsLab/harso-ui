...
var p=H.page('Proto iOS Light');p.layers.slice().forEach(function(l){if(l.name==='Screen/ios/'+APP+'/activity--work'||(/^caption\//.test(l.name)&&l.name.endsWith('/activity--work')))l.remove();});build('activity--work',8);