...
## FIX WAVE 2 — macos
Independent review: /tmp/harso-sk/proto-review/macos.md — PARTIAL (only 16 Light screens scored; vision provider 503s). Read it
whole plus /tmp/harso-sk/proto-review/macos-partial-ledger.json. YOUR job on 'Proto macOS Light' and 'Proto macOS Dark':
1) Finish the review yourself for every screen NOT scored (all Dark + remaining Light), with the FULL-scale question from
/tmp/harso-sk/proto/REVIEW-BRIEF-v2.md, retrying vision on 503 (sleep 20s, up to 5 tries). Record scores.
2) Fix every screen <8 (the review names: conversation--menu 16–18px trailing overhang → align menu right edge to the ⋯
trigger right edge minus 8; Share vs Export glyphs — lead already set Export to square.and.arrow.up.on.square, verify;
customize--profile field heights / label gutter; appearance group gutters). Regenerate with the lane generators
(v2-mac-core/, v2-mac-manage/, v2-mac-settings/) or edit in place; re-shoot; re-score until ≥8.
Report: /tmp/harso-sk/proto/lanes/fix2-macos/FIX-REPORT.md with every id × appearance → score; LANE_RESULT line. Never save.