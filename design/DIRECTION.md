...
4. One type scale: 11 caps labels (0.6 tracking) · 12 meta · 13 chrome · 14 body · 15 title · 22 display (the assistant's single editorial headline). One font authority, the kit (`src/theme.css`, bundled via fontsource, no system fallback chain): Instrument Sans for all interface text, 400-led (body, content, rows) and 500 for labels, headings and emphasis; Geist Mono only for evidence (code, hashes, timestamps as data, versions, keys). `--hk-font` and `--hk-mono` are the only font declarations; components and consumer apps reference those tokens and never name a face.
